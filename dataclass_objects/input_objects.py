...
class expInput() :
    ...
    def __post_init__(self) : 
        ...
        # Have to use batch_size = None and a BatchSampler to speed up sampling rate, avoid elementwise selection
    ...
    def save_state(self) -> None :
        ...
        """Save state of the current experiment input configuration to deploy for later and nullify side effects.
        Note that you can only save one state at a time; performing this will override the last save state. 
        Load this saved state via self.reload_state().
        """
    ...
    def reload_state(self, switch_device : bool = True) -> None :
        ...
        """Reload the state of the experiment input configuration from the save state, generated by self.save_state().
        
        Params:
            switch_device: whether to also change the current device for the network. Defaults to True.
        """
    ...
    def to_device(self, device_type : str, strict : bool = False) -> None :
        ...
        """Move the device of the experiment input configuration to another type, typically CPU or CUDA (GPU).

        Params:
            device_type: the name of the device. Use "cpu" for CPU and "cuda" for GPU/CUDA.
            strict: whether to enforce device switching. If set to true and the switch cannot occur, raises ValueError.

        Raises:
            ValueError: Tried to switch to CUDA and it is not currently available.
            ValueError: No network model structure defined for the model type.
        """
